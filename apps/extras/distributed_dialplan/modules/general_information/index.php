...
function _moduleContent_bak(&$smarty, $module_name)
{
    ...
    switch($accion){
        case "upload":
            $content = uploadGeneralInformation($smarty, $module_name, $local_templates_dir, $pDB);
            break;
        default:
            $content = formGeneralInformation($smarty, $module_name, $local_templates_dir, $pDB);
            break;
    }
    return $content;
}

function uploadGeneralInformation($smarty, $module_name, $local_templates_dir, $pDB)
{
   ...
   if(!$oForm->validateForm($_POST)){
        ...
        if($upload == "1"){
            ...
            if(!$result){
                ...
                return formGeneralInformation($smarty, $module_name, $local_templates_dir, $pDB);
            }else{
    ...
    $contenidoModulo = formGeneralInformation($smarty, $module_name, $local_templates_dir, $pDB);
    return $contenidoModulo;

}


function formGeneralInformation($smarty, $module_name, $local_templates_dir, &$pDB)
{
    $pGeneralInformation = new paloSantoGeneralInformation($pDB);
    $arrFormGeneralInformation = createFieldForm();
    $oForm = new paloForm($smarty,$arrFormGeneralInformation);

    $smarty->assign("UPLOAD", _tr("Save"));
