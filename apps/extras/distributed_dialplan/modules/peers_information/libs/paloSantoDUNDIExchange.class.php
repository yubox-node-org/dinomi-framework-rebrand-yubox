...
class paloSantoDUNDIExchange
{
    ...
    {
        $sql = 'SELECT secret FROM general ORDER BY id LIMIT 1';
    ...
    function createExchangeRequest($local_ip, $comment, $remote_ip, $secret)
    {
        ...
        $r = $this->_localCertificateInfo();
        if (is_null($r)) return NULL;
        list($mac, $outkey, $certfile_path) = $r;
    ...
    {
        return is_array($this->_localCertificateInfo($local_ip));
    }

    private function _localCertificateInfo($local_ip)
    {
        // Averiguar MAC de la interfaz identificada por $local_ip
        ...
        return array($mac, $outkey, $certfile_path);
    }
    ...
    function saveGeneralInformation($local_ip, $data)
    {
        // Averiguar MAC de la interfaz identificada por $local_ip
        $mac = $this->_getMACLocalIP($local_ip);
        if (is_null($mac)) return FALSE;

        // Generar la clave local si no existe
        $outkey = $this->_certnameFromMac($mac);
        $certfile_path = ASTERISK_KEYDIR.'/'.$outkey.'.pub';
        if (!file_exists($certfile_path)) {
            $output = $retval = NULL;
            exec('/usr/sbin/astgenkey -q -n '.escapeshellarg(ASTERISK_KEYDIR.'/'.$outkey).' 2>&1'); // <-- sin extensión .pub
            if ($retval != 0) {
                $this->errMsg = 'astgenkey failed: '.implode("\n", $output);
                return FALSE;
            }
        }

        // Se completan datos ingresados si hay existentes
        $fields = array('organization', 'department', 'locality', 'state',
            'country', 'email', 'phone', 'secret');
        $sql = 'SELECT id, certificate, '.implode(', ', $fields).' FROM general ORDER BY id LIMIT 1';
        $tupla = $this->_DB->getFirstRowQuery($sql, TRUE);
        if (!is_array($tupla)) {
            $this->errMsg = $this->_DB->errMsg;
            return FALSE;
        ...
        foreach ($fields as $k) if (isset($data[$k])) $tupla[$k] = $data[$k];
        if (!isset($tupla['certificate']) || $tupla['certificate'] != $outkey)
            $tupla['certificate'] = $outkey;

        // Los campos se corresponden a entradas de conf
        if (count(array_diff($fields, array_keys($tupla))) > 0) {
            $this->errMsg = _tr('Missing fields').': '.implode(', ', array_diff($fields, array_keys($tupla)));
            return FALSE;
        }

        // Inserción o actualización de datos generales
        $params = array_values($tupla);
        $fieldSQL = array_keys($tupla);
        if (!is_null($id)) {
            // Actualización de datos
            for ($i = 0; $i < count($fieldSQL); $i++)
                $fieldSQL[$i] = $fieldSQL[$i].' = ?';
            $sql = 'UPDATE general SET '.implode(', ', $fieldSQL).' WHERE id = ?';
            $params[] = $id;
        } else {
            // Inserción de datos nuevos
            $sql = 'INSERT INTO general ('.implode(', ', $fieldSQL).') VALUES ('.
                implode(', ', array_fill(0, count($fieldSQL), '?')).')';
        }
        if (!$this->_DB->genQuery($sql, $params)) {
            $this->errMsg = $this->_DB->errMsg;
            return FALSE;
        ...
        // Se actualizan archivos de configuración DUNDI
        unset($tupla['certificate']);
        unset($tupla['id']);
        $tupla['entityid'] = $mac;
        $s = ''; foreach ($tupla as $k => $v) $s .= "$k=$v\n";
        if (FALSE === file_put_contents('/etc/asterisk/dundi_general_custom_elastix.conf', $s)) {
            $this->errMsg = _tr("Unabled write file");
            return FALSE;
        }

        // FIXME: se usa $local_ip que no es necesariamente una IP pública
        $s = <<<DUNDI_MAPPINGS
priv => dundi-priv-canonical,0,IAX2,dundi:${SECRET}@$local_ip/${NUMBER},nopartial
priv => dundi-priv-customers,100,IAX2,dundi:${SECRET}@$local_ip/${NUMBER},nopartial
priv => dundi-priv-via-pstn,400,IAX2,dundi:${SECRET}@$local_ip/${NUMBER},nopartial
DUNDI_MAPPINGS;
        if (FALSE === file_put_contents('/etc/asterisk/dundi_mappings_custom_elastix.conf', $s)) {
            $this->errMsg = _tr("Unabled write file");
            return FALSE;
        }
