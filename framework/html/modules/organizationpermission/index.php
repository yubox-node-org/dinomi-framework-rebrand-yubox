...
{
    function buildGrid($pACL, $customurlprefix, $admin, $is_self)
    {
        $grid = array();
        $this->_buildGridChildren($pACL, $customurlprefix, $this->_menubase, $grid, 0, NULL, $admin, $is_self);
        return $grid;
    }

    private function _buildGridChildren($pACL, $customurlprefix, $children, &$grid, $nlevel, $idparent, $admin, $is_self)
    {
        $total = $enabled = 0;
        ...
        foreach (array_keys($children) as $key) {
            ...
            if ($children[$key]['HasChild']) {
                list($childrentotal, $childrenenabled) = $this->_buildGridChildren(
                    $pACL, $customurlprefix, $children[$key]['children'], $grid, $nlevel + 1, $children[$key]['id'], $admin, $is_self);
...
function reportGroupPermission($smarty, $module_name, $local_templates_dir)
{
    ...
    if (isset($_POST['apply']) && isset($_POST['resource_access']) && is_array($_POST['resource_access'])) {
        applyNewGroupPermissions($smarty, $id_group, $fullmenu, $bIsAdminGroup, $bIsItSelf);
    }
    ...
    $oGrid->setData($oArbol->buildGrid($pACL, '?menu='.$module_name.'&amp;action=assigncustom&amp;id_group='.$id_group, $bIsAdminGroup, $bIsItSelf));
...
function applyNewGroupPermissions($smarty, $id_group, $fullmenu, $is_admin, $is_self)
{
    global $pACL;
    ...
    if ($is_self) {
        $_POST['resource_access'] = array_unique(array_merge(
            $_POST['resource_access'],
            array('organization', 'organizationgroup', 'organizationuser', 'organizationpermission')));
    }
