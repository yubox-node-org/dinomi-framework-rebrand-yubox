...
function _moduleContent(&$smarty, $module_name)
{
    ...
    if (isset($_POST['submit_create_tenant_group'])) {
        ...
        $contenidoModulo = $oForm->fetchForm("$local_templates_dir/organizationgroup.tpl", _tr("New Group"), $arrFillGroup);
        ...
        if (!is_array($arrGroup)) {
            ...
            $contenidoModulo = $oForm->fetchForm("$local_templates_dir/organizationgroup.tpl", _tr('Edit Group') . " \"" . $arrFillGroup['group'] . "\"", $arrFillGroup);
        }
        ...
        if ($oForm->validateForm($_POST)) {
            ...
            if (!empty($pACL->errMsg)) {
                ...
                $contenidoModulo = $oForm->fetchForm("$local_templates_dir/organizationgroup.tpl", _tr("New Group"), $_POST);
            } else {
                header("Location: ?menu=organizationgroup");
            }
        } else {
            // Error
        ...
        if (!is_array($arrGroup)) {
            ...
            if ($oForm->validateForm($_POST)) {
                ...
                if (!$pACL->updateGroup($_POST['id_group'], $groupName, $_POST['description'])) {
                    // Ocurrio algun error aqui
                    $smarty->assign("mb_message", "ERROR: $pACL->errMsg");
                    $contenidoModulo = $oForm->fetchForm("$local_templates_dir/organizationgroup.tpl", _tr("Edit Group"), $_POST);
                } else {
                    header("Location: ?menu=organizationgroup");
                }
            } else {
                ...
                $smarty->assign("id_group", htmlspecialchars($_POST['id_group'], ENT_COMPAT, 'UTF-8'));
                $contenidoModulo = $oForm->fetchForm("$local_templates_dir/organizationgroup.tpl", _tr("Edit Group"), $arrFillGroup);
            }
        }
        ...
        if (!is_array($arrGroup)) {
            ...
            $arrTmp['group']        = substr($arrGroup[0][1], strlen($_SESSION['elastix_user']));   //$arrGroup[0][1];
            $arrTmp['description']  = $arrGroup[0][2];

            $smarty->assign("id_group", htmlspecialchars($_GET['id'], ENT_COMPAT, 'UTF-8'));
            $contenidoModulo = $oForm->fetchForm("$local_templates_dir/organizationgroup.tpl", _tr("View Group"), $arrTmp); // hay que pasar el arreglo
        }
    } else {
        if (isset($_POST['delete'])) {
            //- TODO: Validar el id de organizationgroup
            ...
            } else if (substr($pACL->getGroups($_POST['id_group'])[0][1], 0, strlen($_SESSION['elastix_user'])) == $_SESSION['elastix_user']) {
        ...
        $oGrid->setURL("?menu=organizationgroup");
        ...
        foreach ($arrGroups as $group) {
            ...
            if (substr($group[1], 0, strlen($_SESSION['elastix_user'])) == $_SESSION['elastix_user']) {
                $arrTmp[0] = "&nbsp;<a href='?menu=organizationgroup&action=view&id=" . $group[0] . "'>" . substr($group[1], strlen($_SESSION['elastix_user'])) . "</a>"; //id_organizationgroup   name
