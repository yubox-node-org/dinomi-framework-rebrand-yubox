...
if ! grep "apache ALL = NOPASSWD: /sbin/shutdown" /etc/sudoers >/dev/null 2>&1; then
	echo "apache ALL = NOPASSWD: /sbin/shutdown" >> /etc/sudoers
	echo "apache ALL = NOPASSWD: /sbin/shutdown added to /etc/sudoers"
fi

if ! grep "apache ALL = NOPASSWD: /sbin/service" /etc/sudoers >/dev/null 2>&1; then
	echo "apache ALL = NOPASSWD: /sbin/service" >> /etc/sudoers
	echo "apache ALL = NOPASSWD: /sbin/service added to /etc/sudoers"
fi